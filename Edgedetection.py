...
def start():
    global IMAGE_NAME
    global IMAGE_PATH
    IMAGE_NAME = "1ea9849f-4cfe-4fa6-acb8-a8acd30d2d89"
...
def drawedges(img2, boxes):
    ...
    skel = skel * 255
    # Todo Wichtig entfernen
    # Todo Wichtig entfernen
    # Todo Wichtig entfernen
    # Todo Wichtig entfernen

    skel[458, 603] = [255.0, 255.0, 255.0]
    cv2.imwrite(os.path.join('images' + "/" + IMAGE_NAME + "/", IMAGE_NAME + '_skel' + '.jpg'), skel)
    return skel
...
def find_missing2(crossing_pixel, sections, classified_pixels):
    ...
    for i in range(-radius, +radius + 1):
        for j in range(-radius, +radius + 1):
            ...
            in_sections = False
            if classified_pixels[currentpos] > 0 and not any(
                    [np.any(np.all(np.array(currentpos) == s[0], axis=1)) for s in sections]):
                back = currentpos
                radius2 = 1
                for p in range(-radius2, +radius2 + 1):
                    for q in range(-radius2, +radius2 + 1):
                        ...
                        if classified_pixels[nextpos] > 0 and not any(
        ...
        if len(crossing_pixels_in_range) > 1:
            ...
            for cp in crossing_pixels_in_range:
                ...
                for cpl in crossing_pixels_in_port_sections[cp]:
                    ...
                    #cpl[0].append(np.array([cp[0], cp[1]]))
            ...
            for j in range(0, 5):
                ...
                if (i != 2 or j != 2) and classified_pixels[x + i - 2, y + j - 2] != 0:
                    ...
                    for cpl in crossing_pixels_in_port_sections[crossing_pixel]:
                        if np.array_equal(cpl[0][-2], np.array([x + i - 2, y + j - 2])) or np.array_equal(cpl[0][-1],
                                                                                                          np.array([
                                                                                                                       x + i - 2,
                                                                                                                       y + j - 2])) or (
                        x + i - 2, y + j - 2) == cpl[0][0]:
